...
import f4.bidservice.kafka.producer.Producer;
...
public class BidController {
  ...
  @PostMapping("/order")
  public ResponseEntity<String> bid(RequestEntity<String> hRequest, RequestDTO request, @CookieValue("user") Cookie cookie)
      throws Exception {
    //jwt에서 유저ID 가져오기
//    String info = cookie.getValue().split("\\.")[1];
//    Map<String, Object> map = service.decode(info);
...
//    request = service.bidValidation(request);
    producer.producing("test-topic",hRequest.toString(),hRequest.toString());
